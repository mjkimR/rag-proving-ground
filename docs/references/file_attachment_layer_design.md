...
classDiagram
    ...
    Attachment "1" --* "0..*" SessionAttachment : references
```

### 2.1. `Attachment` (전역 원시 파일 메타데이터)
...
### 2.2. `SessionAttachment` (세션별 바인딩 및 처리 상태)
* **`id`** (UUID, PK): 세션 바인딩 고유 키.
* **`thread_id`** (String, Index): Aegra(LangGraph) 세션 ID.
* **`attachment_id`** (UUID, ForeignKey): `Attachment` 테이블 참조.
...
* *제약 조건*: `(thread_id, attachment_id)` 복합 유니크 제약 조건을 설정하여 하나의 세션 내 동일 파일의 다중 처리를 방지합니다.
...
[ Client ]                   [ FastAPI Backend ]                 [ Database / MinIO ]
    ...
    │                                 │ 3. 기존 해시 존재 여부 검사         │
    │                                 │───────────────────────────────────>│
    │                                 │ <──[Exist] Return Attachment Record│
    │                                 │                                    │
    │                                 │ 4. [NotExist] MinIO 원시 파일 저장 │
    │                                 │───────────────────────────────────>│ (attachments/{sha256})
    │                                 │ 5. [NotExist] DB Attachment 기록   │
    │                                 │───────────────────────────────────>│
    │ 6. Response (Attachment Metadata)│                                    │
    │<────────────────────────────────│                                    │
```
...
* **Response** (`202 Accepted`):
  ```json
  {
    "session_attachment_id": "uuid-of-session-attachment",
...
class AttachmentProcessor(ABC):
    ...
    @abstractmethod
    async def process(self, session_attachment_id: UUID, raw_file_bytes: bytes, filename: str) -> dict:
        """
        비동기 워커에서 실행되어 파일을 처리하고 결과를 메타데이터 딕셔너리로 반환합니다.
        
        Args:
            session_attachment_id: 세션 바인딩 고유 ID
...
1. **원시 업로드 즉시 수행**:
   * 사용자가 파일을 드래그하거나 선택하면 프론트엔드는 즉시 `POST /api/v1/attachments/upload`를 호출합니다.
...
3. **그래프 Safety Gate**:
   * 사용자가 대화창에서 엔터를 쳐서 그래프 실행(`POST /sessions/{thread_id}/runs`)을 시도할 때, Aegra 그래프의 첫 번째 노드(`safety_gate`)는 해당 세션과 연관된 `SessionAttachment` 중 완료되지 않은(`PENDING`, `PROCESSING`) 항목이 존재하는지 확인합니다.
...
1. **세션 만료 정리 (Active/Passive Clean)**:
   * 사용자가 세션 삭제 시 `DELETE /api/v1/sessions/{thread_id}/temp_kbs` (또는 통합 세션 정리 API)가 호출되면 해당 세션에 묶인 `SessionAttachment`와 관련 Qdrant 임시 KB 포인트를 물리 삭제합니다.
...
2. **원시 파일 보존 및 고아 TTL 정리 (Orphaned TTL Garbage Collection)**:
   ...
   * **고아(Orphaned) 상태 정의**: 어떤 `SessionAttachment`나 영구 지식 베이스 문서(`KnowledgeBaseDocument`)에서도 참조되지 않는 `Attachment` 레코드.
   * **지연 GC(Lazy GC) 규칙**:
     * 백그라운드 스케줄러가 주기적으로(예: 매일 자정) 고아 상태인 `Attachment` 목록을 조회합니다.
     * 고아 상태가 된 지 **24시간(또는 설정된 유예 기간)**이 지난 `Attachment` 파일과 레코드를 영구 삭제합니다.
     * 물리 삭제 시 MinIO 원시 경로(`raw-attachments/{sha256}`)의 바이너리 파일과 PostgreSQL의 `Attachment` 레코드를 연쇄 삭제(Cascade)합니다.
