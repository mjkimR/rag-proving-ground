...
본 문서는 **FastStream**과 **Redis Streams**를 기반으로 구축된 비동기 문서 인제스션 및 재처리 파이프라인의 아키텍처와 핵심 설계 패턴을 설명합니다.
...
이를 해결하기 위해, 무거운 연산은 API 요청 흐름 외부로 격리하고 **비동기 Worker 프로세스**로 위임하는 아키텍처를 채택하고 있습니다.
...
[ FastAPI App ] 
    ...
    │ (4) Publish "document.ingest" Message to Redis
    ▼
...
[ Redis Streams Broker ]
    │
    │ (6) Dispatch Message (Consumer Group)
    ▼
[ FastStream Worker Process ]
...
| 컴포넌트 | 기술 스택 | 주요 역할 |
|---|---|---|
| **API API Gateway** | **FastAPI** (Python 3.13) | 파일 검증, MinIO 원본 저장, DB 레코드 생성 및 `QUEUED` 상태 제어, 비동기 큐 퍼블리싱 및 즉시 202 응답 반환 |
| **Message Broker** | **Redis Streams** (`rag-redis`) | Ingest/Reprocess 작업 큐 메시지 중개, Consumer Group을 통한 유실 없는 순차 처리 보장 |
| **Async Worker** | **FastStream (Redis)** | 메시지 구독 및 파이프라인 단계별 처리 프로세스 (API 프로세스와 물리적/논리적 격리) |
...
네트워크 불안정이나 컨테이너 비정상 종료 등으로 인해 메시지 발행에 실패하거나, Worker가 비정상 종료되어 메시지가 유실되는 상황을 복구하기 위해 **Startup Recovery Routine**을 탑재하고 있습니다.

*   **동작 원리**: 
    1. Worker 프로세스가 구동될 때 DB를 스캔하여 생성된 지 **5분이 지난 `QUEUED` 상태**의 문서들을 탐색합니다.
    2. 중단된 문서들을 찾아 `document.ingest` 메시지를 다시 큐에 발행하여 자가 복구(Self-healing)를 수행합니다.
...
*   **네트워크 및 외부 연동 예외 대응**: Worker의 메시지 핸들러는 `tenacity` 패키지의 `@retry(stop=stop_after_attempt(3))` 장치를 도입하여 일시적인 외부 연동(예: OOM, 외부 API 타임아웃, DB 커넥션 병목 등) 실패 시 최대 3회 자동 재시도합니다.
...
Worker 애플리케이션은 FastAPI 라이프사이클 외부에서 독립적으로 가동되므로 FastAPI의 `@app.Depends` 의존성 주입 도구를 사용할 수 없습니다.
*   이를 해결하기 위해 `app.worker.services`에 **`build_pipeline_service()` 수동 DI 팩토리**를 선언하여 DB Repository, History Service, Pipeline Service를 결합도 낮고 일관성 있게 생성합니다.
...
### 5.2. 개발 실행 명령
*   **통합 실행**: `just dev` 명령 실행 시 API 서버와 함께 백그라운드에서 FastStream Worker가 자동으로 시작되며, 정상 종료(SIGINT/SIGTERM) 발생 시 두 프로세스가 안전하게 리소스를 반환하고 함께 내려갑니다.
