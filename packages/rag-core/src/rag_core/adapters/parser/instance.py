...
async def parse_document(
    ...
    *,
    ...
    md5_hash = _parser_cache.store_file(parser_input)
    cached_data = _parser_cache.get_result(md5_hash, parser.name, schema_version=parser.schema_version)
    if cached_data is not None:
        return parser.from_cache_data(cached_data)

    result = await parser.parse(parser_input)
    _parser_cache.store_result(
        md5_hash,
        parser.name,
