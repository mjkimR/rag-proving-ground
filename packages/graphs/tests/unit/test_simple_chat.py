...
def test_simple_chat_success(mocker):
    ...
    mock_llm.invoke.return_value = AIMessage(content="Hello from mock LLM")
...
def test_simple_chat_default_model(mocker):
    ...
    # Mock get_llm_model
    mock_llm = mocker.MagicMock()
    mock_llm.invoke.return_value = AIMessage(content="Hello default")
    mock_get_llm = mocker.patch("rag_graphs.simple_chat.get_llm_model")
    mock_get_llm.return_value = mock_llm
...
def test_simple_chat_invalid_model(mocker):
    # Mock get_model_options
    mock_options = mocker.patch("rag_graphs.simple_chat.get_model_options")
    mock_options.return_value = {
        "llm_models": ["gpt-oss-20b"],
