    ...
    async def enrich_chunks_with_page_content(self, session: AsyncSession, chunks: list[RetrievedChunk]) -> None:
        ...
        for chunk in chunks:
            ...
            if isinstance(p_ids, list):
                page_ids_to_fetch.update(p_ids)

        if not page_ids_to_fetch:
            return

        pages = await self.get_by_page_ids(session, list(page_ids_to_fetch))
        page_map = {page.page_id: page.content for page in pages}
        for chunk in chunks:
            p_ids = chunk.metadata.get("page_ids")
            if isinstance(p_ids, list) and p_ids:
                contents = [page_content for p_id in p_ids if (page_content := page_map.get(p_id)) is not None]
                if contents:
                    chunk.page_content = "\n\n".join(contents)
